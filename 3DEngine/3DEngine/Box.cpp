...
bool Box::SpawnControlWindow(int id, Graphics& gfx) noexcept
{
	...
	if ( ImGui::Begin( ("Box "s + std::to_string(id)).c_str(),&open ) )
	{
		...
		const auto sid = ImGui::SliderFloat("Specular Intensity", &materialConstants.specularIntensity, 0.05f, 4.0f);
		const auto spd = ImGui::SliderFloat("Specular Power", &materialConstants.specularPower, 1.0f, 200.0f);
		dirty = cd || sid || spd;
